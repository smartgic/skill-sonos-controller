...
def discovery(self):
    ...
    if not self.speakers:
        ...
        self.log.info("{} device(s) found".format(len(self.speakers)))
...
def get_category(self, service, category):
    ...
    self.log.warning("{} category not found for {} service".format(category, service))
            ...
            if service in subscription.lower():
                ...
                if service in auth:
                    if not os.path.isfile(os.getenv("HOME") + TOKEN_FILE):
                        self.log.warning("{} requires authentication".format(service))
...
def _valid_music_source(self, speaker):
    ...
    try:
        if speaker.is_playing_tv or speaker.is_playing_line_in:
            msg = (
                "unsupported action on {} speaker when music "
